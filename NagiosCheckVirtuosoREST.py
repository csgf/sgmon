...
#    def __init__(self,wT,cT,oF,q,e,lL=logging.DEBUG):
    ...
    def getInputParameters(self,argV=sys.argv):
        ...
        try:
         #   opts,args = getopt.getopt(argV[1:],"hq:e:o:c:w:",["--help","--query","--endpoint","--outputfile","--critical","--warning"])
            opts,args = getopt.getopt(argV[1:],"hk:l:e:o:c:w:",["--help","--keyword","--limit","--endpoint","--outputfile","--critical","--warning"])
        ...
        for opt,arg in opts:
            ...
            if opt == '-h':
                print argV[0]+"-k keyword -e endpoint -l limit -o outputfile -c critical_threshold -w warning_threshold"
                print "Number of keyword is limited to one. Offset is not supported, and forced to 0"
                sys.exit(NagiosCheck.UNKNOWN)
    ...
    def validateLimit(self):
        ...
        To be coherent with the check, limit needs to be above warning threshold 
        '''
    
        if self.limit < self.criticalThreshold:
            self.logger.error("Limit(%d) is below critical threshold, which means that max number of records returned would raise a critical status for the check\n Reconsider thresholds and limit parameters for the check." %(self.limit))
            sys.exit(NagiosCheck.UNKNOWN)
        elif self.limit < self.warningThreshold:
            self.logger.error("Limit (%d) is below warning threshold, which means that max number of records returned would raise a warning status for the check\n Reconsider thresholds and limit parameters for the check." %(self.limit))
            sys.exit(NagiosCheck.UNKNOWN)
        else:
            self.logger.info("Limit (%d) is above warning threshold, max number of results expected is coherent with the given thresholds " %(self.limit))

    def runCheck(self):

        queryTimeout=15
        restURL="%s?keyword=%s&limit=%d&offset=0" %(self.endpoint,self.keyword,self.limit)
        ...
        try:
            ...
            self.logger.error("%s exceeds %d seconds timeout !" %(restURL,queryTimeout))
            return -1
