...
class NagiosCheckVirtuoso(NagiosCheck):
    ...
    def getInputParameters(self, argV=sys.argv):
        ...
        try:
            opts, args = getopt.getopt(argV[1:], "hq:e:o:c:w:",
                            ["--help", "--query", "--endpoint", "--outputfile",
                             "--critical", "--warning"])
        ...
        for opt, arg in opts:
            ...
            if opt == '-h':
                print argV[0] + ("-q query -e endpoint -o outputfile"
                                 "-c critical_threshold -w warning_threshold")
                sys.exit(NagiosCheck.UNKNOWN)
            elif opt in ("-q", "--query"):
                inputDict['query'] = arg
                #print query
    ...
    def runCheck(self):
        ...
        self.logger.info("Performing query \n %s \n on endpoint\n %s"
                          % (self.query, self.endpoint))

        try:
            with NagiosTimeout(queryTimeout):
                result = sparql.query().convert()
        except NagiosTimeout.Timeout:
            self.logger.error("Endpoint %s exceeds %d seconds timeout !"
                               % (queryTimeout, self.endpoint))
            return -1
        except Exception, err:
            self.logger.error(("Impossible to perform given query: %s\n"
                               "on endpoint %s\n %s"
                               % (self.query, self.endpoint, err)))
            return -1
