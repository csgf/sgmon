...
class NagiosCheckApp(NagiosCheck):
    ...
    def __init__(self,warnThr,critThres,outFil,jmxDF,jmxLF,testSz,logLev=logging.DEBUG):
    ...
    def getInputParameters(self,argV=sys.argv):
        ...
        inputDict={}
        filename=argV[0]
        helpMessage="""%s -c <critical_threshold_rate> -w <warning_threshold_rate> -o <output_file> -j <jmx_properties_file> -l <jmx_log_file> -n <number of users>""" %(filename)

        try:
            opts,args=getopt.getopt(argV[1:],"hc:w:o:j:l:n:",["--help","--critical","--warning","--outfile","--jmx","--jmx-log","--number-of-users"])
	...
	"""
        ...
        jMeterCmd="%s/jmeter -n -t %s -l %s -JLoopSize=%s" %(jMeterPrefix,self.jmxDescFile,self.jmxLogFile,self.testSize)
        ...
        try:
            ...
            # DEBUG MODE jMeterOutcome=subprocess.check_output(jMeterCmd.split(),stderr=subprocess.STDOUT)
        ...
        if (jMeterOutcome != ""):
            self.logger.debug("jMeter  %s exit status is %d" %(jobDesc,jMeterOutcome))
            # DEBUG MODE self.logger.debug("jMeter  %s exit status is %s" %(jobDesc,jMeterOutcome))
            return 0
        else:
    ...
    def checkOutcome(self):
        import re
        # 
	# se TUTTE le righe del file danno 200, allora torna 0 (OK), altrimenti torna il numero di errori
	...
	userResponses={}
	for line in results:
            check=re.match('\d+,\d+,([\w ]+),([\w ]+),[\w ]+,(oar-user \d-\d)',line)
	    if check:
		    ...
		    userResponses[user]=1
       		    error+=line
		    self.logger.error("Error accessing [%s]: %s" %(pageTitle,line[:-1]))
    ...
    def analyzeScore(self,checkScore,failures,submitted):
        ...
        if checkScore >= self.criticalThreshold:
            self.outMsg="CRITICAL: failed to login for %d user%s out of %d attempted (%.2f %% failed)" %(failures,({True: '', False: 's'}[submitted < 2]),
                                                                                                         submitted,checkScore*100)
            self.result=NagiosCheck.CRITICAL
        elif checkScore >= self.warningThreshold:
            self.outMsg="WARNING: failed to login for %d user%s out of %d attempted (%.2f %% failed)" %(failures,({True: '', False: 's'}[submitted < 2]),
                                                                                                        submitted,checkScore*100)
            self.result=NagiosCheck.WARNING
        elif checkScore >= 0.0:
            self.outMsg="OK: login achieved for %d user%s out of %d attempted (%.2f %% failed)." %(submitted - failures,
                                                                                            ({True: '', False: 's'}[submitted < 2]),submitted,checkScore*100)
