...
class NagiosCheckeTokenServer(NagiosCheck):
    ...
    def getInputParameters(self, argV=sys.argv):
        ...
        try:
            opts, args = getopt.getopt(argV[1:],
                             "hu:o:c:w:",
                            ["--help", "--urlsfile", "--outputfile",
                             "--critical", "--warning"])
        except getopt.GetoptError:
            print argV[0] + (" -u <url file list> -o <outputfile>"
    ...
    def runCheck(self):
        ...
        if self.score >= self.criticalThreshold:
            self.outMsg = ("eTokenServer CRITICAL: failed to retrieve proxy "
                           "for %d targets out of %d given (%.2f %% failed)"
                           % (self.failures, lineCounter, self.score * 100))
            self.result = NagiosCheck.CRITICAL
        elif self.score >= self.warningThreshold:
            self.outMsg = ("eTokenServer WARNING: failed to retrieve proxy for"
                           "%d targets out of %d given (%.2f %% failed)"
                           % (self.failures, lineCounter, self.score * 100))
            self.result = NagiosCheck.WARNING
        elif self.score >= 0.0:
            self.outMsg = ("eTokenServer OK: %d proxies retrieved out of "
                           "%d given (%.2f %% failed)."
                           % (lineCounter - self.failures, lineCounter,
                              self.score * 100))
            self.result = NagiosCheck.OK
        else:
            self.outMsg = ("eTokenServer status UNKNOWN:"
                           "Unable to get eTokenServer health")
    ...
    def getProxy(self, url):
        ...
        try:
            cert = OpenSSL.crypto.load_certificate(
                        OpenSSL.crypto.FILETYPE_PEM, buff2string)
        except OpenSSL.crypto.Error:
            check.logger.error(("unable to read certificate downloaded"
                                "from %s\n-----\n" % (url)))
            return 1
        ...
        dateString = ("%s-%s-%s %s:%s:%s UTC"
                       % (dateASN[0:4], dateASN[4:6], dateASN[6:8],
                          dateASN[8:10], dateASN[10:12], dateASN[12:14]))

        check.logger.debug("got a certificate for %s, valid until %s"
                         % (certCN, dateString))
...
if __name__ == "__main__":
    ...
    check.logger.debug(("Starting test with warning threshold %f"
    ...
    try:
        ...
        check.logger.error("Unable to read urls from input file %s"
                          % (inputDictionary['eTokenURLsFile']))
